	...
	)
{
	...
	, validateWebsiteOptions: function validateWebsiteOptions(){
		var weboptionsvalid = true;
		var focused = false;
		var selectrequired = $('select.required-entry');
		selectrequired.each(function(index){
			if($(this).val() == ''){
				weboptionsvalid = false;
				if(!focused){
					focused = true;
					$('.product-details-weboptions-title').after('<div class="validation-advice" style="">Please configure required options/accessories.</div>');
					$('html, body').animate({
			        scrollTop: $('.product-details-weboptions-title').offset().top
			    }, 2000);
				}
				$(this).addClass('validation-failed');
				$(this).after('<div class="validation-advice" style="">This is a required field.</div>')
			}
		});
		var inputrequired = $('input.required-entry');
		...
		var ulrequired = $('ul.required-entry');
		...
		return weboptionsvalid;
	}
	, removeValidationText: function removeValidationText(){
		$('.validation-failed').removeClass('validation-failed');
		$('div.validation-advice').remove();
		return true;
	}
	...
	,	addToCart: function addToCart (e)
		{
			...
			var self = this
			,	cart_promise;
			this.removeValidationText();
			if(!this.validateWebsiteOptions()){
				return;
			}
			//Check if they all havevalues... else return
			//if they have values add it to the options custom field
